...
// Get shift display label from shift letter
export function getShiftLabel(shiftLetter) {
  const labels = {
    A: 'Shift A (Day: 6AM-6PM)',
    B: 'Shift B (Night: 6PM-6AM)',
    C: 'Shift C (Day: 6AM-6PM)',
    D: 'Shift D (Night: 6PM-6AM)',
  };
  return labels[shiftLetter] || shiftLetter;
}

// Get shift time-of-day from letter
export function getShiftTimeOfDay(shiftLetter) {
  return ['A', 'C'].includes(shiftLetter) ? 'day' : 'night';
}

// Get today's date adjusted for shift (for default date in wizard)
