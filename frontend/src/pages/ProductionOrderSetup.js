...
import {
  createProductionOrder as apiCreateProductionOrder,
...
function ProductionOrderSetup({ lines, skus }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const orders = await apiGetActiveOrders();
        if (cancelled) return;

        if (productionOrder?.productionOrderId) {
          const matchingOrder = orders.find(
            (order) => String(order.id) === String(productionOrder.productionOrderId)
          );
        ...
        if (!cancelled) setLoadingRecovery(false);
      }
    })();
    return () => { cancelled = true; };
  }, [clearOrder, navigate, productionOrder]);
  ...
  const handleResumeOrder = (order) => {
    setOrder({
      ...
      skuId: order.sku_id,
      skuName: order.product_name,
      productCode: order.product_code,
      poNumber: order.po_number || '',
      bestBuyCode: order.best_buy_code,
      shift: order.shift,
      shiftDate: order.shift_date,
  ...
  const canProceed = useCallback(() => {
    switch (step) {
      ...
      case 2: return poNumber.trim().length > 0;
      case 3: return bestBuyCode.trim().length > 0;
      case 4: return selectedShift !== null;
      case 5: return shiftDate !== '';
      case 6: return true;
      default: return false;
    }
  }, [step, selectedLine, selectedSku, poNumber, bestBuyCode, selectedShift, shiftDate]);
  ...
  if (showRecovery && activeOrders.length > 0) {
    return (
      <div className="selection-page" style={{ position: 'relative' }}>
        ...
        <div style={{ textAlign: 'center', width: '100%', maxWidth: '650px' }}>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: '0.5rem' }}>
            Active Production Orders Found
          </h2>
          <p style={{ color: 'var(--text-light)', marginBottom: '1.5rem' }}>
            Resume an existing order or start a new one
          </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.5rem' }}>
            {activeOrders.map(order => (
              <div
                ...
                style={{ padding: '1rem 1.25rem', cursor: 'pointer', textAlign: 'left', transition: 'border-color 0.2s' }}
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: '1.05rem' }}>{order.product_name}</div>
                    <div style={{ fontSize: '0.85rem', color: 'var(--text-light)', marginTop: '0.25rem' }}>
                      {order.line_name} | Shift {order.shift} | {order.shift_date}
  ...
  const renderPoNumber = () => (
    <div style={{ textAlign: 'center', width: '100%', maxWidth: '500px' }}>
      ...
      <p style={{ color: 'var(--text-light)', marginBottom: '2rem' }}>
        Enter the PO number from the planning system
      </p>
      <input
        className="premium-input"
        type="text"
