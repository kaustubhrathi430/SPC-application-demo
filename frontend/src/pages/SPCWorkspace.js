...
function SPCWorkspace({ lines, skus }) {
  ...
  // Load chart data filtered by active freezer
  ...
  }, [productionOrder, sku, line, activeFreezer]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await withRetry(() => updateMeasurement(editingId, {
        ...
        await withRetry(() => createMeasurement({
          sku_id: productionOrder.skuId,
          line_id: productionOrder.lineId,
          freezer_number: activeFreezer,
          operator_initials: form.operator_initials,
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: '0.9rem', opacity: 0.95 }}>
              {inControlCount} readings in control this shift on Freezer #{activeFreezer} — Great job!
            </div>
          </div>
      ...
      <div className="main-content">
        ...
        <div className="form-section">
          <div className="premium-card form-card">
            ...
            <form onSubmit={handleSubmit}>
              ...
              {editingId && (
                <button type="button" className="premium-btn danger" style={{ marginTop: '0.5rem' }}
                  onClick={() => { setEditingId(null); setForm({ operator_initials: form.operator_initials, thickness_value: '', weight_value: '', coating_value: '', adjustments: '' }); }}>
                  Cancel Edit
                </button>
              )}
            </form>
          ...
          <div className="premium-card">
            <div className="card-header">
              <h3>Recent Entries — Freezer #{activeFreezer}</h3>
              <span className="timestamp-display">{recentEntries.length} entries</span>
            </div>
            <div className="entries-list">
              {recentEntries.length === 0 ? (
                <div className="empty-state">
                  <p>No entries yet for Freezer #{activeFreezer} this shift.</p>
                  ...
                  <div key={entry.id} className="entry-item">
                    <div className="entry-header">
                      <div>
                        ...
                        <div className="entry-date">{formatDate(entry.recorded_at)} | Freezer #{entry.freezer_number}</div>
                      </div>
                      <div className="entry-actions">
                        <button className="icon-btn" title="Edit" onClick={() => handleEdit(entry)}>Edit</button>
                      </div>
                    </div>
        ...
        <div className="charts-section">
          <div style={{
            ...
            Charts for Freezer #{activeFreezer}
          </div>
          <SPCChart title={`${sku.thickness_label} — Freezer #${activeFreezer}`} data={chartData} valueKey="thickness_value" limits={thicknessLimits} />
          <SPCChart title={`${sku.weight_label} — Freezer #${activeFreezer}`} data={chartData} valueKey="weight_value" limits={weightLimits} />
          <SPCChart title={`${sku.coating_label} — Freezer #${activeFreezer}`} data={chartData} valueKey="coating_value" limits={coatingLimits} />
        </div>
      </div>
