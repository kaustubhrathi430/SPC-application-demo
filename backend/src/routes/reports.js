  ...
  try {
    ...
    const reportData = {
      ...
      measurements: measurements.map(m => ({
        ...
        pump: m.pump_number || 1,
        time: m.recorded_at,
        thickness: m.thickness_value,
        weight: m.weight_value,
        coating: m.coating_value,
        status: m.status_overall || 'in_control',
        acknowledged_at: m.alert_acknowledged_at,
    ...
    const result = await client.query(
      ...
      [
        ...
        measurements.length, resolvedBestBuyCode, resolvedPoNumber, productionOrder?.id || production_order_id || null,
...
router.get('/:id/pdf', async (req, res) => {
  try {
    ...
    const safePoNumber = (report.po_number || `report-${report.id}`)
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=shift-report-${safePoNumber || report.id}-${report.shift_date}.pdf`
    );
    doc.pipe(res);
    ...
    doc.text(`PO Number: ${report.po_number || 'N/A'}`);
    doc.text(`Best Buy Code: ${report.best_buy_code || 'N/A'}`);
    ...
    if (measurements.length > 0) {
      ...
      const colWidths = [48, 38, 36, 52, 52, 52, 60, 40, 125];
      const headers = ['Time', 'Frzr', 'Pump', 'Thickness', 'Weight', 'Coating', 'Status', 'Op.', 'Adjustments'];
      ...
      for (const m of measurements) {
        ...
        const statusLabel = m.status === 'ooc'
          ? (m.acknowledged_at ? 'OOC ACK' : 'OOC')
          : m.status === 'warning'
            ? (m.acknowledged_at ? 'WARN ACK' : 'WARNING')
            : 'OK';
        const rowData = [
          time, String(m.freezer), String(m.pump || 1),
